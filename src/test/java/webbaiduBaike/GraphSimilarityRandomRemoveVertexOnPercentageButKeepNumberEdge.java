...
public class GraphSimilarityRandomRemoveVertexOnPercentageButKeepNumberEdge {
    ...
    static {
        try {
            ...
            System.out.printf(
                    "graphe de taille %s et avec %s",
                    reference.edgeSet().size(), reference.vertexSet().size()
            );
    ...
    void benchmarkMethodOnDensities(
        ...
        for (Double density : densities) {
            ...
            double result = func.compute(reference, g, pr2);  // <-- récupère le résultat
            double elapsed = (System.nanoTime() - start) / 1e9;
            System.out.printf(
                    "graphe de taille %s et avec %.2f de sommets et avec %s en moins comparés en %.2fs ; Similarité : %.5f",
                    g.vertexSet().size(), density * 100, g.edgeSet().size(), elapsed, result
            );
        }
    }
    ...
    public void testVeoSimilarityOnVertexChangePercentage1() throws IOException {
        double[] densities = {0.10, 0.20, 0.50};
        String rootDir = "output/vertexChangePercentageButKeepEdgeAt14MillionsMaxGraphs";
